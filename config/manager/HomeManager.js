...
function getProfessionals(cb) {
    config.acquire(function (err, connection) {
        ...
        var jsonArray = [];
        var query = "SELECT PM.*,PIM.IMG_NAME FROM [PROFESSIONAL_MSTR] PM LEFT JOIN [PROFESSIONAL_IMAGE_MSTR] PIM ON PM.USERNAME = PIM.[USER_NAME] ORDER BY PM.ID DESC";
        const request = new Request(query, (err, rowCount, rows) => {
            if (err) {
                console.log(err)
            } else {}
...
function getProfbyId(v1, cb) {
    config.acquire(function (err, connection) {
        ...
        var jsonArray = [];
        var query = `SELECT PM.*,PIM.IMG_NAME FROM [PROFESSIONAL_MSTR] PM 
                LEFT JOIN [PROFESSIONAL_IMAGE_MSTR] PIM 
...
function getProfessional(v1, cb) {
    config.acquire(function (err, connection) {
        ...
        console.log(v1);
        var jsonArray = [];
        var query = `SELECT * FROM PROFESSIONAL_MSTR WHERE [USER_ID] = ${v1}`;
        const request = new Request(query, (err, rowCount, rows) => {
            if (err) {
                console.log(err)
            } else {}
