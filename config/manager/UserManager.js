        ...
        var query = `SELECT 
                ...
                T1.BUYER_PROF,T1.BUYER_PROF_ID,T2.SELLER,T2.SELLER_PROF,T2.SELLER_PROF_ID
                        ...
                        LEFT JOIN(
                            ...
                            LEFT JOIN PROFESSIONAL_MSTR PM ON PM.ID = US.PROF_ID
                        ...
                        LEFT JOIN(
                            ...
                            LEFT JOIN PROFESSIONAL_MSTR PM ON PM.ID = BS.PROF_ID
...
function getServices(v1,cb) {
    config.acquire(function (err, connection) {
        ...
        var jsonArray = [];
        var query = "SELECT * FROM USER_SERVICE US LEFT JOIN PROFESSIONAL_MSTR PM ON PM.ID = US.PROF_ID WHERE US.SERVICE_TYPE <> 'Principle Agent' AND US.TRAN_ID='"+v1.TranId+"' ";
        const request = new Request(query, (err, rowCount, rows) => {
            if (err) {
                console.log(err)
            } else {
